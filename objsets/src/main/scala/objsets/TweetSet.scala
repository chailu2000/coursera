...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if (p(elem))
      remove(elem).filterAcc(p, acc.incl(elem))
    else
      remove(elem).filterAcc(p, acc)
  }
  ...
  override def union(that: TweetSet): TweetSet = {
    var tset: TweetSet = that
    foreach(tw => tset = tset.incl(tw))
    tset
  }

  override def mostRetweeted: Tweet = {
    var rtmax: Tweet = elem
    foreach(tw => if (tw.retweets > rtmax.retweets) rtmax = tw)
    rtmax
  }
  
  override def leastRetweeted: Tweet = {
    var rtmin: Tweet = elem
    foreach(tw => if (tw.retweets <= rtmin.retweets) rtmin = tw)
    rtmin
  }
  def max(tw1: Tweet, tw2: Tweet): Tweet = if (tw1.retweets < tw2.retweets) tw2 else tw1

  override def descendingByRetweet: TweetList = {
    orderByRetweetAcc(Nil, this)
  }

  def orderByRetweetAcc(tl: TweetList, tset: TweetSet): TweetList = {
    if (tset.isInstanceOf[Empty]) tl
    else {
      val tw = tset.leastRetweeted
      orderByRetweetAcc(new Cons(tw, tl), tset.remove(tw))
    }
  }
  /**
   * The following methods are already implemented
